...
# from pyon.runner._db import ListType
#
#
...
#     def __init__(self, view):
#         super(DBManager, self).__init__(view)
#         #self.con = view.all()
#         self.table_name = view.manager_kwargs['table_name']
...
#         filtered = self._do_filter(my_filter, self.view.all())
#         if len(filtered) == 0:
...
#
#     def sort(self, sort_by):
#         return sorted(list(self.view.all()), key=lambda item: item[sort_by])
#
#     def unique(self, field_name):
#         values = []
#         for f in self.view.all():
#             try:
#                 v = f[field_name]
#                 if v not in values:
#                     values.append(v)
